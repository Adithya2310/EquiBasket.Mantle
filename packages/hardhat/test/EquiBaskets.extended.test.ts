...
describe("EquiBaskets Extended Test Coverage", function () {
  ...
  describe("Liquidity Pool - Extended Coverage", function () {
    ...
    beforeEach(async function () {
      ...
      await basketVault.connect(fundCreator).depositCollateral(techBasketId, { value: ethers.parseEther("400") });
      await basketVault.connect(fundCreator).mintBasket(techBasketId, ethers.parseEther("0.1"));

      // Add liquidity to pool with native MNT
      await basketToken1.connect(fundCreator).approve(await pool.getAddress(), ethers.parseEther("0.1"));
      await pool.connect(fundCreator).addLiquidity(ethers.parseEther("0.1"), { value: ethers.parseEther("50") });
    });
    ...
    it("Should reject removing more shares than owned", async function () {
      ...
      await expect(pool.connect(fundCreator).removeLiquidity(excessShares)).to.be.revertedWithCustomError(
        pool,
        "InvalidShares",
      );
    });
    ...
    it("Should reject removing zero shares", async function () {
      await expect(pool.connect(fundCreator).removeLiquidity(0)).to.be.revertedWithCustomError(pool, "InvalidAmount");
    });
    ...
    it("Should allow owner to collect fees", async function () {
      ...
      await expect(pool.connect(deployer).collectFees()).to.emit(pool, "FeesCollected");
    ...
    it("Should revert swap if pool has insufficient MNT", async function () {
      ...
      await basketVault.connect(user1).depositCollateral(techBasketId, { value: ethers.parseEther("400") });
      ...
      await expect(pool.connect(user1).swapBasketForMnt(basketIn)).to.be.revertedWithCustomError(
        pool,
        "InsufficientPoolMnt",
      );
    });
  ...
  describe("Basket Registry - Deactivation/Reactivation", function () {
    ...
    it("Should allow creator to deactivate basket", async function () {
      ...
      await expect(basketRegistry.connect(fundCreator).deactivateBasket(techBasketId))
    ...
    it("Should allow owner to deactivate basket", async function () {
      await expect(basketRegistry.connect(deployer).deactivateBasket(techBasketId))
        .to.emit(basketRegistry, "BasketActiveStateChanged")
        .withArgs(techBasketId, false);
    ...
    it("Should reject deactivation by non-creator/non-owner", async function () {
      await expect(basketRegistry.connect(user1).deactivateBasket(techBasketId)).to.be.revertedWithCustomError(
        basketRegistry,
        "NotBasketCreator",
      );
    });
    ...
    it("Should allow reactivating a deactivated basket", async function () {
      ...
      await expect(basketRegistry.connect(fundCreator).reactivateBasket(techBasketId))
        .to.emit(basketRegistry, "BasketActiveStateChanged")
  ...
  describe("Basket Vault - Edge Cases", function () {
    ...
    it("Should reject burning more than user's debt", async function () {
      ...
      await expect(basketVault.connect(user1).burnBasket(techBasketId, excessiveBurn)).to.be.revertedWithCustomError(
        basketVault,
        "InsufficientDebt",
      );
    });
    ...
    it("Should allow setting liquidator address", async function () {
      ...
      await expect(basketVault.connect(deployer).setLiquidator(liquidatorAddr))
    ...
    it("Should handle partial burns correctly", async function () {
      ...
      expect(remainingDebt).to.be.closeTo(initialDebt - burnAmount, ethers.parseEther("0.0001"));
    });
  });
