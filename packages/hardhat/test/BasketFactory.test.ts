...
import { BasketRegistry, BasketOracle, BasketVault, BasketFactory } from "../typechain-types";
...
describe("BasketFactory with Native MNT", function () {
  ...
  describe("Basket Creation via Factory", function () {
    it("Should create basket with token in one transaction", async function () {
      const tx = await basketFactory
        .connect(fundCreator)
        .createBasketWithToken(TECH_BASKET_ASSETS, TECH_BASKET_WEIGHTS, "Tech Giants", "eTECH");
    ...
    it("Should allow creating multiple baskets", async function () {
      ...
      await basketFactory
        .connect(user1)
        .createBasketWithToken(COMMODITY_BASKET_ASSETS, COMMODITY_BASKET_WEIGHTS, "Precious Metals", "eMETAL");
    ...
    it("Should revert if vault not set", async function () {
      ...
      await expect(
        newFactory.createBasketWithToken(TECH_BASKET_ASSETS, TECH_BASKET_WEIGHTS, "Tech Giants", "eTECH"),
  ...
  describe("Native MNT Deposits", function () {
    ...
    it("Should return native MNT on withdrawal", async function () {
      ...
      await expect(basketVault.connect(user1).withdrawCollateral(basketId, withdrawAmount)).to.changeEtherBalance(
        user1,
        withdrawAmount,
      );
    });
  ...
  describe("Collateral Ratio Management", function () {
    ...
    it("Should enforce minimum collateral ratio for minting", async function () {
      ...
      await expect(basketVault.connect(user1).mintBasket(basketId, tooMuchMint)).to.be.revertedWithCustomError(
        basketVault,
  ...
  describe("Edge Cases", function () {
    it("Should handle zero deposits gracefully", async function () {
      ...
      await expect(basketVault.connect(user1).depositCollateral(1n, { value: 0 })).to.be.revertedWithCustomError(
        basketVault,
        "InvalidAmount",
      );
    });
    ...
    it("Should prevent minting tokens without registered token", async function () {
      ...
      await basketRegistry
        .connect(fundCreator)
        .createBasket(TECH_BASKET_ASSETS, TECH_BASKET_WEIGHTS, "Unregistered", "UNREG");
