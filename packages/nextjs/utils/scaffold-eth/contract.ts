...
// Get the chain IDs that are actually present in deployedContracts
type DeployedChainIds = keyof typeof contractsData;

// Check if contracts are missing for the configured chain
type IsContractDeclarationMissing<TYes, TNo> = DeployedChainIds extends never ? TYes : TNo;

// If the configured chain has contracts, use those, otherwise use GenericContractsDeclaration 
type ContractsDeclaration = IsContractDeclarationMissing<GenericContractsDeclaration, typeof contractsData>;

// Get contracts - use first chain ID's contracts directly
type Contracts = (typeof contractsData)[5003];
